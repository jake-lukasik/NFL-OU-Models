---
title: "Using Machine Learning to Predict NFL Betting Outcomes"
...
Here, we simply join the scores table to the team_id field so that we can have the home and away teams' IDs on the scores table
...
Before we move onward, we should define what exactly a 'spread' is in sports betting, as you will see it come up occasionally. It is not explicitly used in our models, but for the future, if we seek to build a model that aims to predict spread, it will be useful information;
...
## Over/Under Classification Models
...
So according to the gradient boosting algorithm, the top games to bet on are Tennessee @ Houston, Detroit @ Indianapolis, and Minnesota @ Chicago.

#### Predicting via XGBoost:

Repeating the same process seen in the previous model predictions:

First, because of the way that XGBoost model works, we have to first create a matrix with the week 12 matchup infor, then it is the same process

```{r}
week12.mx <- sparse.model.matrix(~ ., data = week12_matchups)
train_columns <- colnames(dtrain)
# ensure only the common columns remain
week12.mx <- week12.mx[, colnames(week12.mx) %in% train_columns]
week12_dmatrix <- xgb.DMatrix(week12.mx)
```

```{r}
# make predictions using the trained XGBoost model on the new data 
week12_pred_OU_xgb <- predict(final_model, newdata = week12_dmatrix)

# convert the probabilities to "Over" or "Under" based on a threshold
week12_xgb_pred_OU <- ifelse(week12_pred_OU_xgb > 0.383, "Over", "Under")

week12_xgb_pred_prob <- ifelse(week12_xgb_pred_OU == "Over", 
                               round((0.5+(week12_pred_OU_xgb - 0.383))*100,3), 
                               round((0.5+(0.383 - week12_pred_OU_xgb))*100,3)
                               )
  
  
# Consolidate the predictions into a new dataframe
xgb_prediction_df <- data.frame(
  Home = week12_matchups$home_id,   # home team ID
  Away = week12_matchups$away_id,   # away team ID
  Line = week12_matchups$over_under_line,  # Over/Under line
  Prediction = week12_xgb_pred_OU,  # prediction: "Over" or "Under"
  Confidence = week12_xgb_pred_prob # probability based off predict output
)

# display the predictions in a nicely formatted table using kable
xgb_prediction_df %>%
  kable(format = "html", caption = "<span style='color: black;'>O/U Predictions for Week 12 Games By XGBoost Model") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  column_spec(4, bold = TRUE, color = "white", background = "#0073e6") %>%
  column_spec(5, bold = TRUE, color = "black", background = "#f0f0f0") %>%
  add_header_above(c("Game Information" = 3, "Prediction" = 2))
...
Due to the nature of the XGBoost algorithm and how the cutoff is at 0.383 for the most accurate predictions, and not just 0.5, it is very hard to quantify the confidence (proability), so for this table, we have had to do some feature engineering for most accurate confidence levels scaled at a reasonable value.

Doing the same thing to find the three best bets:

```{r}
top_xgb_locks <- xgb_prediction_df %>%
  arrange(desc(`Confidence`)) %>% 
  head(3) %>%  
  select(Home, Away, `Prediction`)
top_xgb_locks
...
#### Predicting via Logistic Regression:

Finally, doing the same predictions with the logistic regression model:

```{r}
week12_pred_OU_logit <- predict(model_lr, newdata = week12_matchups, type = "response")

# convert the probabilities to "Over" or "Under" based on a threshold
week12_logit_pred_OU <- ifelse(week12_pred_OU_logit > 0.418, "Over", "Under")

week12_logit_pred_prob <- ifelse(week12_logit_pred_OU == "Over", 
                               round((0.5+(week12_pred_OU_logit - 0.418))*100,3), 
                               round((0.5+(0.418 - week12_pred_OU_logit))*100,3)
                               )
...
logit_prediction_df <- data.frame(
  ...
  Away = week12_matchups$away_id,   # away team ID
  Line = week12_matchups$over_under_line,  # Over/Under line
  Prediction = week12_logit_pred_OU,  # prediction: "Over" or "Under"
  Confidence = week12_logit_pred_prob # probability based off of prediction output
)

# display the predictions in a nicely formatted table using kable
logit_prediction_df %>%
  kable(format = "html", caption = "<span style='color: black;'>O/U Predictions for Week 12 Games By Logistic Regression Model") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  column_spec(4, bold = TRUE, color = "white", background = "#0073e6") %>%
  column_spec(5, bold = TRUE, color = "black", background = "#f0f0f0") %>%
  add_header_above(c("Game Information" = 3, "Prediction" = 2))
...
And as well, find the highest confidence games

```{r}
top_logit_locks <- logit_prediction_df %>%
  arrange(desc(`Confidence`)) %>% 
  head(3) %>%  
  select(Home, Away, `Prediction`)
top_logit_locks
...
Now, lets compare our models to the actual result of these games:

First, let us create and read in a dataframe with the actual O/U results:

```{r}
week12_outcomes <- read.csv("week12_outcomes.csv")
```

```{r}
# merge the week12_outcomes dataframe with all prediction dataframes

merged_df <- week12_outcomes %>%
  left_join(select(rf_prediction_df, Home, Away, Prediction), by = c("Home", "Away")) %>%
  rename("RF Prediction" = Prediction) %>%
  left_join(select(gbm_prediction_df, Home, Away, Prediction), by = c("Home", "Away")) %>%
  rename("GBM Prediction" = Prediction) %>%
  left_join(select(xgb_prediction_df, Home, Away, Prediction), by = c("Home", "Away")) %>%
  rename("XGBoost Prediction" = Prediction) %>%
  left_join(select(logit_prediction_df, Home, Away, Prediction), by = c("Home", "Away")) %>%
  rename("Logistic Regression Prediction" = Prediction)

# create a formatted table with conditional coloring based on prediction match
merged_df %>%
  kable(format = "html", caption = "<span style='color: black;'>Week 12 Predictions vs Actual Outcomes") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
  column_spec(4, bold = TRUE, color = "white", background = "#0073e6") %>%
  # apply green or red background color for prediction matches
  column_spec(5, background = case_when(
    merged_df$`RF Prediction` == merged_df$Outcome ~ "rgba(0, 255, 0, 0.8)", 
    merged_df$`RF Prediction` != merged_df$Outcome ~ "rgba(255, 0, 0, 0.8)"  
  )) %>%
  column_spec(6, background = case_when(
    merged_df$`GBM Prediction` == merged_df$Outcome ~ "rgba(0, 255, 0, 0.8)", 
    merged_df$`GBM Prediction` != merged_df$Outcome ~ "rgba(255, 0, 0, 0.8)"  
  )) %>%
  column_spec(7, background = case_when(
    merged_df$`XGBoost Prediction` == merged_df$Outcome ~ "rgba(0, 255, 0, 0.8)", 
    merged_df$`XGBoost Prediction` != merged_df$Outcome ~ "rgba(255, 0, 0, 0.8)" 
  )) %>%
  column_spec(8, background = case_when(
    merged_df$`Logistic Regression Prediction` == merged_df$Outcome ~ "rgba(0, 255, 0, 0.8)", 
    merged_df$`Logistic Regression Prediction` != merged_df$Outcome ~ "rgba(255, 0, 0, 0.8)"  
  )) %>%
  add_header_above(c("Game Information" = 4, "Model Outcomes" = 4))

```

From this, we can see that the Random forest model performed the best with the blind predictions for Week 12 of NFL matchups, going 6/11 with an accuracy of 54.54%.
...
Another potential area to explore is the use of every predictor in our models. Some predictors may not be as effective as others in the model and may even point the model in the wrong direction. In the future, variable selection methods could be explored to further enhance the performance of our models. Also, as mentioned earlier in this project the tuning of hyperparameters was used to improve prediction accuracy. The scope of tuning of these parameters was relatively small and could be widened in the future at the cost of deficiency and runtime. Clearly, this project still has possibilities to expand and become even more effective. 
